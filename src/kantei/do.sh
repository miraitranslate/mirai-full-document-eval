...
perl 1_extract_uris.pl ${index_dir%/} > URIs.txt
./2_download_en.sh URIs.txt $html_en_dir
newest=`date -d "${oldest}01 +${num_months} month" +%Y%m`
python3 3_extract_body.py $html_en_dir $html_ja_dir ${json_dir%/}/$oldest-$newest.json
